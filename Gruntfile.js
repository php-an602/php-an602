module.exports = function (grunt) {
        ...
        shell: {
            buildAssets: {
                command: function() {
                    ...
                    let dirSep = isWin() ? "\\" : '/';
                    let jsFile = `static${dirSep}js${dirSep}an602-*.js`;
                    let cssFile = `static${dirSep}css${dirSep}an602-*.css`;
                    return `${rm} ${jsFile} ${sep} ${rm} ${cssFile} ${sep} ${delAssets} ${sep} cd protected ${sep} php yii asset an602/config/assets.php an602/config/assets-prod.php`;
                }
            },
            buildSearch: {
                command: function() {
            ...
            testRun: {
                command: function() {
                    ...
                    let rootTestPath = `${base}/protected/an602/tests`;

                    let testPath = rootTestPath;
                    if(moduleName) {
                        testPath = `${base}/protected/an602/modules/${moduleName}/tests`;
                    }
    ...
    /**
     * Will create a new migration into the protected/an602/migrations directory
     *
