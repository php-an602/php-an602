...
use an602\modules\queue\driver\Sync;
...
/**
 * ConfigController allows inital configuration of an602.
...
class ConfigController extends Controller
{
    ...
    {
        $form = new \an602\modules\installer\forms\ConfigBasicForm();
    ...
    {
        $form = new \an602\modules\installer\forms\UseCaseForm();
    ...
    {
        $form = new \an602\modules\installer\forms\SecurityForm();
    ...
    public function actionSampleData()
    {
        ...
        $form = new \an602\modules\installer\forms\SampleDataForm();
        ...
        if ($form->load(Yii::$app->request->post()) && $form->validate()) {
            ...
            if (Yii::$app->getModule('installer')->settings->get('sampleData') == 1) {
                ...
                $adminImage = new \an602\libs\ProfileImage($admin->guid);
                ...
                $profileImage = new \an602\libs\ProfileImage($userModel->guid);
                ...
                $profileImage2 = new \an602\libs\ProfileImage($userModel2->guid);
                ...
                $post->content->visibility = \an602\modules\content\models\Content::VISIBILITY_PRIVATE;
                $post->save();

                // Switch Identity
                Yii::$app->user->switchIdentity($userModel);

                $comment = new \an602\modules\comment\models\Comment();
                ...
                $comment2 = new \an602\modules\comment\models\Comment();
                ...
                // Create Like Object
                $like = new \an602\modules\like\models\Like();
    ...
    public function actionAdmin()
    {
        ...
        $form = new \an602\compat\HForm($definition);
    ...
    public function actionFinish()
    {
        if (Yii::$app->settings->get('secret') == "") {
            Yii::$app->settings->set('secret', \an602\libs\UUID::v4());
        }

        \an602\libs\DynamicConfig::rewrite();
