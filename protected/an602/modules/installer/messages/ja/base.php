...
return array (
  ...
  'Here you can decide how new, unregistered users can access an602.' => 'an602にアクセスした外部ユーザーや、新規ユーザーの閲覧権限を決めてください。',
  'Hostname' => 'ホスト名',
  ...
  'an602 is very flexible and can be adjusted and/or expanded for various different applications thanks to its’ different modules.  The following modules are just a few examples and the ones we thought are most important for your chosen application.<br><br>You can always install or remove modules later. You can find more available modules after installation in the admin area.' => 'an602は非常に柔軟性があり、モジュールが異なるため, さまざまなアプリケーションに合わせて調整および/または拡張できます。 以下のモジュールはほんの一例であり, 選択したアプリケーションにとって最も重要であると私たちが考えたものです。<br><br>モジュールは後でいつでもインストールまたは削除できます。 インストール後, 管理エリアで利用可能なモジュールをさらに見つけることができます。',
  'an602 uses cron jobs to execute scheduled or to run potentially heavy tasks such as sending out email summaries and notifications or optimizing the search index.' => 'an602はcronジョブを使用して、スケジュールされたタスクを実行したり、電子メールの要約や通知の送信、検索インデックスの最適化など、潜在的に重いタスクを実行したりします。',
  'I want to use an602 for:' => 'あなたがan602を利用する形態は：',
  'In order to activate this feature, it is necessary to edit the an602 configuration and, potentially, the configuration of the WebServer.' => 'この機能を有効にするには、an602構成を編集する必要があり、場合によっては Web サーバーの構成も編集する必要があります。',
  ...
  'On an602\'s side, you will need to edit the configuration file <code>{configFile}</code> and add the following block:' => 'an602側では、構成ファイル<code>{configFile}</code>を編集して、次のブロックを追加する必要があります。',
  'Open the crontab of an602/PHP process user e.g. <code>{user}</code>.' => 'an602/PHPプロセスユーザーのcrontabを開きます。
<code>{user}</code>',
  ...
  'The name of the database you want to run an602 in.' => 'an602を構築したいデータベースの名前を入れてください。',
  'This overview shows all system requirements of an602.' => '下記はan602のすべての必須要件です。',
...
ドキュメントに記載されているジョブスケジューリングの設定に問題がある場合は、サーバー管理者にサポートを依頼してください。',
  ...
  'Yay! I\'ve just installed an602 :sunglasses:' => 'an602をインストールしたぞ！',
