 ...
 */
an602.module('notification', function (module, require, $) {
    ...
    OverviewWidget.prototype.init = function () {
        var that = this;
        event.on('an602:notification:filterApplied', function (evt, form) {
            evt.preventDefault();
    ...
    NotificationDropDown.prototype.init = function (update) {
        ...
        var that = this;
        event.on('an602:modules:notification:live:NewNotification', function (evt, events, update) {
        ...
        event.on('an602:modules:notification:UpdateTitleNotificationCount', function (evt, events, update) {
    ...
    NotificationDropDown.prototype.updateCount = function ($count) {
        ...
        event.trigger('an602:notification:updateCount', [$count]);
    ...
    var updateTitle = function ($count) {
        ...
        // Mail Module triggers also `an602:modules:notification:UpdateTitleNotificationCount` on New Messages
        if (an602.modules.mail && an602.modules.mail.notification && an602.modules.mail.notification.getNewMessageCount) {
            $count += an602.modules.mail.notification.getNewMessageCount();
        }
    ...
    var handleFilterChanges = function () {
        ...
        filterForm.on('click', 'label', function (evt) {
            ...
            event.trigger('an602:notification:filterApplied', filterForm);
        });
    };
