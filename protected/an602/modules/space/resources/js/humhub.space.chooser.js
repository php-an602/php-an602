...
an602.module('space.chooser', function (module, require, $) {
    ...
    SpaceChooser.prototype.initEvents = function () {
        ...
        event.on('an602:ready', function () {
            if (!space.isSpacePage()) {
                that.setNoSpace();
            }
        }).on('an602:space:changed', function (evt, options) {
            that.setSpace(options);
        }).on('an602:space:archived', function (evt, space) {
            that.removeItem(space);
        }).on('an602:space:unarchived', function (evt, space) {
            that.prependItem(space);
        }).on('an602:modules:content:live:NewContent', function (evt, liveEvents) {
