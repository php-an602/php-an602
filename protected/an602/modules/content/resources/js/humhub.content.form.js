 ...
 * @type Function
 */
an602.module('content.form', function(module, require, $) {
    ...
    CreateForm.prototype.init = function() {
        ...
        if(!module.config['disabled']) {
            $('#contentFormBody').on('click.an602:content:form dragover.an602:content:form', function(evt) {
            ...
            $('#contentFormBody').find('.an602-ui-richtext').trigger('disable');
        }
    };
    ...
    CreateForm.prototype.submit = function(evt) {
        ...
        var that = this;
        evt.block = 'manual';
        event.trigger('an602:content:beforeSubmit', this);
        client.submit(evt).then(function(response) {
            that.$.find(".preferences, .fileinput-button").show();
            $('.contentForm_options .preferences, .fileinput-button').show();
            if(!response.errors) {
                event.trigger('an602:content:newEntry', response.output, this);
                event.trigger('an602:content:afterSubmit', response.output, this);
                that.resetForm();
            } else {
    ...
    /**
     * Todo: this is post form only, this needs to be added to post module perhaps by calling $form.trigger('an602:form:clear');
     * @returns {undefined}
     */
    CreateForm.prototype.resetForm = function() {
        ...
        $('#contentFormBody').find('.an602-ui-richtext').trigger('clear');
    };
