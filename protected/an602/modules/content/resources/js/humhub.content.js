...
an602.module('content', function (module, require, $) {
    ...
    Content.prototype.remove = function () {
        ...
        return new Promise(function (resolve, reject) {
            that.$.animate({height: 'toggle', opacity: 'toggle'}, 'fast', function () {
                that.$.remove();
                event.trigger('an602:modules:content:afterRemove', that);
                resolve(that);
            });
        });
    };
    ...
    var permalink = function(evt) {
        ...
        event.one('an602:ready', function () {
            modal.global.close();
        });
    };
    ...
    var submitMove = function(evt) {
        modal.submit(evt).then(function(response) {
           if(response.success) {
               ...
               event.trigger('an602:content:afterMove', response);
           }
        });
    };
