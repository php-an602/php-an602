...
class m160220_013525_contentcontainer_id extends Migration
{
    ...
    public function up()
    {
        ...
        $this->update('content', [
            'contentcontainer_id' => new Expression('(SELECT id FROM contentcontainer WHERE class=:spaceModel AND pk=space_id)', [':spaceModel' => \an602\modules\space\models\Space::class])
                ], ['IS NOT', 'space_id', new Expression('NULL')]
        );

        // Set content container for user content
        $this->update('content', [
            'contentcontainer_id' => new Expression('(SELECT id FROM contentcontainer WHERE class=:userModel AND pk=user_id)', [':userModel' => \an602\modules\user\models\User::class])
