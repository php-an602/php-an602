...
class m160507_202611_settings extends Migration
{
    ...
    public function up()
    {
        ...
        $rows = (new Query())
                ...
                ->from('user_setting')
                ->leftJoin('contentcontainer', 'user_setting.user_id = contentcontainer.pk AND contentcontainer.class=:class', [':class' => \an602\modules\user\models\User::class])
        ...
        $rows = (new Query())
                ->select("*, contentcontainer.id as cid")
                ->from('space_setting')
                ->leftJoin('contentcontainer', 'space_setting.space_id = contentcontainer.pk AND contentcontainer.class=:class', [':class' => an602\modules\space\models\Space::class])
                ->andWhere('contentcontainer.id IS NOT NULL')
                ->all();
        foreach ($rows as $row) {
            $this->insertSilent('contentcontainer_setting', [
