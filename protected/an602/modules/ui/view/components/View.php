...
use an602\libs\Html;
use an602\modules\web\pwa\widgets\LayoutHeader;
use an602\modules\web\pwa\widgets\SiteIcon;
use an602\widgets\CoreJsConfig;
...
class View extends \yii\web\View
{
    ...
    public function endBody()
    {
        ...
        if (Yii::$app->params['installed']) {
            if (Yii::$app->getSession()->hasFlash('view-status')) {
                ...
                $value = trim($value);
                $this->registerJs('an602.modules.ui.status.' . $type . '("' . $value . '")', View::POS_END, 'viewStatusMessage');
            }
    ...
    private function registerViewContext()
    {
        if (!empty(static::$viewContext)) {
            $this->registerJs('an602.modules.ui.view.setViewContext("' . static::$viewContext . '")', View::POS_END, 'viewContext');
        }
    }
    ...
    protected function flushJsConfig($key = null)
    {
        if (!empty($this->jsConfig)) {
            $this->registerJs("an602.config.set(" . json_encode($this->jsConfig) . ");", View::POS_BEGIN, $key);
            $this->jsConfig = [];
        }
    }
