 ...
 *
 * @package an602\modules\stream\actions
 * @since 1.7
 */
trait LegacyStreamTrait
{
    ...
    private static function renderLegacyWallEntry(ContentActiveRecord $record, $options = [], $partial = true)
    {
        ...
        if ($partial) {
            return Yii::$app->controller->renderPartial('@an602/modules/content/views/layouts/wallEntry', [
        ...
        return Yii::$app->controller->renderAjax('@an602/modules/content/views/layouts/wallEntry', [
            'content' => $record->getWallOut($options),
            'jsWidget' => $jsWidget,
            'entry' => $record->content
        ]);
