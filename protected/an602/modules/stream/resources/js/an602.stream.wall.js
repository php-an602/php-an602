...
an602.module('stream.wall', function (module, require, $) {
    ...
    WallStream.prototype.initEvents = function () {
        var that = this;
        this.on('an602:stream:beforeLoadEntries.wallStream', function () {
            $('#btn-load-more').hide();
        }).on('an602:stream:afterAddEntries.wallStream', function (evt, resp, res) {
        ...
        }).on('an602:stream:lastEntryLoaded.wallStream', function () {
            $('#btn-load-more').hide();
        });

        event.on('an602:content:beforeSubmit.wallStream', function (evt, html) {
            that.submitLock = true;
        });

        event.on('an602:content:afterSubmit.wallStream', function (evt, html) {
            that.prependEntry(html, true).then(function() {
                that.submitLock = false;
            });
        });

        event.on('an602:content:afterMove.wallStream', function (evt, response) {
    ...
    WallStreamFilter.prototype.init = function() {
        ...
        event.on('an602:topic:updated.wallStream', $.proxy(this.onTopicUpdated, this));
    ...
    WallStreamFilter.prototype.initTopicPicker = function() {
        ...
        if(topicPicker) {
            topicPicker.$.on('change', function() {
                ...
                // Note the stream init is triggered by the an602:topic:updated event
